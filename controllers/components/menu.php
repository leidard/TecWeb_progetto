...
function _menu($currentPage, $logged = false){
    ...
    $out = $template;
    $tagPagine = array (
        'Home' => '<a class="menu_link" xml:lang="en" lang="en" href="index.php">Home</a>',
        ...
        'Login' => '<a class="menu_link" xml:lang="en" lang="en" href="login.php">Login</a>'
    );
    ...
    foreach ($tagPagine as $name => $ref) {
        if ($currentPage === $name){
            if($name === 'Home')
                $str.= "<li><span class=\"pag_corrente\" xml:lang=\"en\" lang=\"en\">$name</span></li>";
            else
